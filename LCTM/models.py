...
class ChainModel(CoreModel):
    def __init__(self, skip=1, init_w_=None, init_pw_=None, **kwargs):
        CoreModel.__init__(self, name="SC-Model", **kwargs)

        self.potentials["class_prior"] = priors.class_prior()
#        self.potentials["temporal_prior"] = priors.temporal_prior(length=30)
        self.potentials["unary"] = unary.framewise_unary(name="unary",init_w=init_w_)
        
        # self.potentials["pw2"] = pw.pairwise("pw2", skip*2)
        if skip: self.potentials["pw"] = pw.pairwise(skip, name="pw", init_pw=init_pw_)
...
class PretrainedEnergiesModel(CoreModel):
    def __init__(self, skip=0, prior=0, segmental=False, **kwargs):
        CoreModel.__init__(self, name="Pretrained-Energies-Model", **kwargs)

        self.potentials["pre"] = unary.pretrained_unary()
        if skip: self.potentials["pw"] = pw.pretrained_pairwise(skip=skip)
