...
class pretrained_pairwise(CorePotential):
    def __init__(self, pretrained_weights, skip=1, name='pw'):
        self.skip = skip
        self.name = name
    ...
    def compute(self, model, Xi, score):
        return compute_pw(score, pretrained_weights, self.skip)


class pairwise(CorePotential):
    def __init__(self, skip=1, name='pw', init_pw=None):
        self.skip = skip
        self.name = name
        self.init_pw = init_pw


    def init_weights(self, model):
        if self.init_pw != None:
            return self.init_pw
        else:
            return np.random.randn(model.n_nodes, model.n_nodes)
        # return np.eye(n_nodes, dtype=np.float64)
