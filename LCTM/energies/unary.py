...
class framewise_unary(CorePotential):
    def __init__(self, name='unary', init_w=None):
        self.name = name
        self.init_w=init_w

    def init_weights(self, model):
        if self.init_w != None:
            #print('init with pretrained w')
            #print('inside init weights', self.init_w)
            return self.init_w
        else:
    ...
    def compute(self, model, Xi, score):
        return score + np.dot(model.ws[self.name], Xi) #@ in python 3.5
